...
export default function ManualEntryPage() {
  ...
  const getMetricDisplayName = (key: string) => {
    return metrics.find(m => m.key === key)?.display_name || key
  }

  const getAvailableMetrics = (currentMetric: string) => {
    const usedMetrics = measurements.map(m => m.metric).filter(m => m !== currentMetric)
    return metrics.filter(m => !usedMetrics.includes(m.key))
  }
