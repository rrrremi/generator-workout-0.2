...
-- ============================================================================
-- Include metrics_catalog JOIN in the RPC function to eliminate
-- the separate query for display names
-- ============================================================================
...
BEGIN
  ...
  SELECT 
    l.metric,
    COALESCE(mc.display_name, l.metric) as display_name,  -- Include display name from catalog
    l.latest_value,
    l.unit,
    l.latest_date,
    l.source,
  ...
  LEFT JOIN metrics_catalog mc ON l.metric = mc.key  -- JOIN with catalog (no N+1!)
...
-- PERFORMANCE NOTES
-- ============================================================================
-- Before: 2 queries (RPC + catalog fetch)
-- After:  1 query (RPC with JOIN)
